...
local s_CommunityUpdate =
{
	...
	function OnGameEvent_tank_spawn( params )
	{
		...
		if ( m_iTankCount == 1 )
		{
			// Only runs in Versus mode, excludes Taaannnk!! Mutation.
			...
			// Does not support maps with more than 1 Tank spawn (e.g. finales) - but could in the future if necessary.
	...
	function ApplyRoundChanges()
	{
		...
		{
			// No fixes were applied, so delete the new Survivor/Infected activator
